...
package com.consol.citrus.http.client.jakarta;

import java.net.URI;
import java.util.Optional;

import com.consol.citrus.context.TestContext;
import com.consol.citrus.endpoint.jakarta.AbstractEndpoint;
 ...
 * Synchronous response messages are cached in local memory and receive operations are able to fetch responses from this
 * cache later on.
 *
...
public class HttpClient extends AbstractEndpoint implements Producer, ReplyConsumer {
    ...
    public void send(Message message, TestContext context) {
        ...
        HttpEntity<?> requestEntity = getEndpointConfiguration().getMessageConverter().convertOutbound(httpMessage,
                getEndpointConfiguration(), context);
        ...
        try {
            ...
            if (getEndpointConfiguration().getBinaryMediaTypes().stream().anyMatch(mediaType -> mediaType.includes(accept))) {
                response = getEndpointConfiguration().getRestTemplate().exchange(URI.create(endpointUri), method, requestEntity,
                        byte[].class);
            } else {
                response = getEndpointConfiguration().getRestTemplate().exchange(URI.create(endpointUri), method, requestEntity,
                        String.class);
            ...
            correlationManager.store(correlationKey, getEndpointConfiguration().getMessageConverter().convertInbound(response,
                    getEndpointConfiguration(), context));
            ...
            Message responseMessage = getEndpointConfiguration().getMessageConverter().convertInbound(
                    new ResponseEntity<>(e.getResponseBodyAsString(), e.getResponseHeaders(), e.getStatusCode()),
                    getEndpointConfiguration(), context);
    ...
    private String getEndpointUri(HttpMessage httpMessage) {
        if (getEndpointConfiguration().getEndpointUriResolver() != null) {
            return getEndpointConfiguration().getEndpointUriResolver().resolveEndpointUri(httpMessage,
