...
package com.consol.citrus.endpoint.jakarta;
...
/**
 * Endpoint builder interface. All endpoint builder implementations do implement this interface in order to build
 * endpoints using a fluent Java API.
 *
...
public interface EndpointBuilder<T extends Endpoint> {
    ...
    default T build(CitrusEndpoint endpointAnnotation, ReferenceResolver referenceResolver) {
        ...
        for (CitrusEndpointProperty endpointProperty : endpointAnnotation.properties()) {
            Method propertyMethod
                    = ReflectionUtils.findMethod(this.getClass(), endpointProperty.name(), endpointProperty.type());
            if (propertyMethod != null) {
                if (!endpointProperty.type().equals(String.class)
                        && referenceResolver.isResolvable(endpointProperty.value())) {
                    ReflectionUtils.invokeMethod(propertyMethod, this,
                            referenceResolver.resolve(endpointProperty.value(), endpointProperty.type()));
                } else {
                    ReflectionUtils.invokeMethod(propertyMethod, this,
                            TypeConversionUtils.convertStringToType(endpointProperty.value(), endpointProperty.type()));
                }
            }
        }
    ...
    default T build(Properties endpointProperties, ReferenceResolver referenceResolver) {
        for (Map.Entry<Object, Object> endpointProperty : endpointProperties.entrySet()) {
            Method propertyMethod = ReflectionUtils.findMethod(this.getClass(), endpointProperty.getKey().toString(),
                    endpointProperty.getValue().getClass());
            if (propertyMethod != null) {
    ...
    /**
     * Resolves all available endpoint builders from resource path lookup. Scans classpath for endpoint builder meta
     * information and instantiates those builders.
     * 
     * @return
     */
    static Map<String, EndpointBuilder<?>> lookup() {
        Map<String, EndpointBuilder<?>> builders
                = new HashMap<>(TYPE_RESOLVER.resolveAll("", TypeResolver.TYPE_PROPERTY_WILDCARD));
    ...
    /**
     * Resolves endpoint builder from resource path lookup with given resource name. Scans classpath for endpoint
     * builder meta information with given name and returns instance of the builder. Returns optional instead of
     * throwing exception when no endpoint builder could be found.
     *
